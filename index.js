...
// Initialize an empty array to store category names and expenditures
const categoryData = [];

// Function to add category expenditure and update the pie chart
const addCategoryExpenditure = () => {
  const category = document.getElementById('expenditureCategory').value;
  const expenditure = parseFloat(prompt(`Enter the amount spent on ${category}:`));
  ...
  // Add the category and expenditure to the array
  categoryData.push({ category, expenditure });

  // Update the pie chart
  updatePieChart();
};

// Function to update the pie chart with current category expenditures
const updatePieChart = () => {
  const labels = categoryData.map(item => item.category);
  const data = categoryData.map(item => item.expenditure);
  ...
  if (window.myPieChart) {
    ...
    window.myPieChart = new Chart(pieChartCtx, {
      type: 'pie',
      data: {
        labels: labels,
        datasets: [{
...
document.getElementById('addCategoryButton').addEventListener('click', addCategoryExpenditure);

// Call the updatePieChart function initially to render an empty pie chart
updatePieChart();
